...
volatile uint8_t inst = 0x00;
volatile DirState current_state = DIR_STOP;
...
//----------------------------------------------------------------------------------------------
//Initiate clock and gpios and UART
//---------------------------------------------------------------------------------------------

//typedef unsigned char uint8_t;
//typedef unsigned char bool;

void run_state_machine(uint8_t cmd);
...
void_run_state_machine(uint8_t cmd)
{
    switch(cmd){
        case 0:
            current_state = DIR_STOP;
            dir_A_forward(); dir_B_forward();
            break;
        case 1:
            current_state = DIR_FORWARD;
            dir_A_forward(); dir_B_reverse();
            break;
        default:
            switch(current_state) {
                case DIR_STOP:      dir_A_forward(); break;
                case DIR_FORWARD:   dir_B_forward(); break;
                case DIR_LEFT:      dir_B_reverse(); break;
            }
            break;
    }
